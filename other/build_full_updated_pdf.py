...
out_pdf_path = r'C:\Users\ADI\Downloads\MINDSPARK-Mockups-Review.pdf' # overwrite target or save to new
final_pdf_path = r'A:\MS\MINDSPARK — Final Mockups.pdf'
downloads_pdf_path = r'C:\Users\ADI\Downloads\MINDSPARK-Mockups-Review.pdf'

print(f"Opening source PDF: {src_pdf_path}")
src_doc = fitz.open(src_pdf_path)
out_doc = fitz.open()

TOTAL_SCREENS = 64

# Flash Anzan 4 Interactive Screens
flash_screens = [
    {
...
def render_flash_mockup_page(doc, info, screen_idx, total_screens=64):
    ...
    # Breadcrumb
    page.insert_text(fitz.Point(36.0, 33.4 + 6), info['breadcrumb'], fontname="hebo", fontsize=8.5, color=(26/255, 56/255, 41/255))
    # Title
    page.insert_text(fitz.Point(36.0, 52.0 + 8), info['title'], fontname="hebo", fontsize=16.0, color=(15/255, 23/255, 42/255))
    # Path
    page.insert_text(fitz.Point(36.0, 70.5 + 6), info['path'], fontname="courier", fontsize=8.0, color=(100/255, 116/255, 139/255))
    # Description
...
cover_page.insert_text(fitz.Point(36.0, 290.0), "MINDSPARK", fontname="helv", fontsize=44.0, color=(26/255, 56/255, 41/255))
cover_page.insert_text(fitz.Point(36.0, 325.0), "Final Mockups — Design Review", fontname="helv", fontsize=18.0, color=(15/255, 23/255, 42/255))
...
# 2. Page 1: Student Section Divider
...
student_div.insert_text(fitz.Point(36.0, 190.0), "STUDENT", fontname="helv", fontsize=36.0, color=(26/255, 56/255, 41/255))
...
# Helper to copy a page and update footer
def copy_mockup_page(src_p, dest_doc, new_idx, total=64):
    ...
    # Blank out old footer area and re-write with new screen index
...
# 6. Remaining Student screens (from original pages 12 to 29, which were screens 11 to 28)
for p_idx in range(12, 30):
    screen_counter += 1
    copy_mockup_page(src_doc[p_idx], out_doc, screen_counter, TOTAL_SCREENS)

# 7. Admin Section Divider (original page 30)
admin_div = out_doc.new_page(width=1116.0, height=400.0)
admin_div.draw_rect(fitz.Rect(0, 0, 1116.0, 400.0), fill=(1, 1, 1), width=0)
admin_div.insert_text(fitz.Point(36.0, 190.0), "ADMIN", fontname="helv", fontsize=36.0, color=(26/255, 56/255, 41/255))
...
# 8. Admin screens (from original pages 31 to 62, which were screens 29 to 60)
for p_idx in range(31, 63):
    screen_counter += 1
    copy_mockup_page(src_doc[p_idx], out_doc, screen_counter, TOTAL_SCREENS)

print(f"Total screens added: {screen_counter}")
print(f"Total pages in new PDF: {len(out_doc)}")
...
import shutil
shutil.copy2(final_pdf_path, r'C:\Users\ADI\Downloads\MINDSPARK-Mockups-Review.pdf')
shutil.copy2(final_pdf_path, r'C:\Users\ADI\Downloads\MINDSPARK-Final-Mockups.pdf')
print("Updated: C:\\Users\\ADI\\Downloads\\MINDSPARK-Mockups-Review.pdf")
print("Saved: C:\\Users\\ADI\\Downloads\\MINDSPARK-Final-Mockups.pdf")
