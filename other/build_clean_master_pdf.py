...
base_orig_pdf = r'A:\MS\MINDSPARK — Final Mockups.pdf' # or original source
final_pdf_path = r'A:\MS\MINDSPARK_Master_Review.pdf'
downloads_pdf_path = r'C:\Users\ADI\Downloads\MINDSPARK-Mockups-Review.pdf'

TOTAL_SCREENS = 64

# Open the original PDF (stored in backup or freshly opened)
# Let's open the 63-page version or copy the exact pages
...
# 6. Remaining Student screens (original pages 16 to 33 in the expanded doc or 12 to 29 in original)
# Let's find pages starting from Results flow
# In src_doc, let's search for "STUDENT › RESULTS FLOW" page
results_start_idx = -1
for i, page in enumerate(src_doc):
    t = page.get_text()
    if 'STUDENT' in t and 'RESULTS FLOW' in t and 'Screen 1 — List page' in t:
        results_start_idx = i
        break

print(f"Results flow starts at page index: {results_start_idx}")

# Results flow + Student profile (18 screens total)
for p_idx in range(results_start_idx, results_start_idx + 18):
    screen_counter += 1
    copy_mockup_page(src_doc[p_idx], out_doc, screen_counter, TOTAL_SCREENS)
...
# 8. Admin screens (32 screens total)
admin_start_idx = -1
for i, page in enumerate(src_doc):
    t = page.get_text()
    if 'ADMIN' in t and 'ADMIN DASHBOARD' in t and 'Admin Dashboard — layout draft' in t:
        admin_start_idx = i
        break

print(f"Admin dashboard starts at page index: {admin_start_idx}")
for p_idx in range(admin_start_idx, admin_start_idx + 32):
    screen_counter += 1
    copy_mockup_page(src_doc[p_idx], out_doc, screen_counter, TOTAL_SCREENS)

print(f"Total screens in clean PDF: {screen_counter}")
print(f"Total pages in clean PDF: {len(out_doc)}")
...
print("Successfully generated clean 67-page master PDF without duplicates!")
