...
### Postgres Query Optimizer in Test Environments (Empty Tables)
When testing index coverage locally or on staging databases with only a few rows, `EXPLAIN ANALYZE` may unexpectedly show a `Seq Scan` or an index scan on the Primary Key instead of the new targeted index (like `students_institution_idx`). This is not a failure of the index. The query planner calculates that scanning a 1-row table (or reading a unique PK) has mathematically lower cost than a non-unique B-tree index lookup followed by a heap fetch. To force the planner to use the target index for evidence, either insert enough rows to cross the cost threshold, or rely on `pg_indexes` presence as proof of availability.

### React 19 forwardRef Deprecation
- **Location:** src/components/shared/sr-announcer.tsx
- **Issue:** React 19 deprecates orwardRef (refs are now passed as standard props). The SRAnnouncer component uses it and will trigger a warning. Refactoring to ({ ref, ...props }) is recommended before it becomes a hard error in future versions.

### Verification of Claims and CVEs
- **Pattern:** Specific version numbers and CVE details need an actual search with a source, not recalled from training.
- **Issue:** Confident and precise-sounding recall is not a substitute for checking. There were multiple instances (Next.js codemod output, CVE-2024-46982 vs CVE-2025-29927 patch versions) where hallucinated numbers were provided. Always execute a web search or check the actual CLI output instead of guessing.


The test suite \cross-tenant-isolation.test.ts\ depends on live remote Supabase Auth for user creation. It is subject to rate-limit exhaustion (and subsequent test hangs/timeouts) during heavy local iteration.

### Vitest JSDOM environment hangs on chained Supabase requests
Test files that execute multiple sequential or chained Supabase API calls (especially Auth/Admin endpoints) will silently hang or timeout if run within the default \jsdom\ vitest environment. The mocked fetch or event loop in JSDOM fails to resolve them. Always add \// @vitest-environment node\ to the top of Supabase integration tests to avoid false-positive timeouts.
