...
- Read/Write access to the project root (generates `.ai-style-rules.md` and optionally `CLAUDE.md` / `AGENTS.md`)
...
- **Strong signal** -> clarify/grill: near-even split, or semantic fork on a core dimension
- **Small-project exception**: sources <= 50, "3 vs 2" is NOT a majority -> clarify it
...
| Option | Mechanism |
|---|---|
| **1** Soft hook (recommended) | Write `@.ai-style-rules.md` reference into project `CLAUDE.md` / `AGENTS.md` |
...
When `.ai-style-rules.md` is in context (loaded via `CLAUDE.md` / `AGENTS.md`), every code-writing task must open with a **compliance declaration** in the reasoning chain, naming the exemplar being followed and the DONTs being avoided.
...
- **Per-Turn Enforcement** - When hooked via instructions, every code-writing task opens with a compliance declaration naming the exemplar followed and the DONTs avoided.

## Output Specification

- `.ai-style-rules.md` at project root (with commit fingerprint + scale tier in header)
- Optionally `CLAUDE.md` / `AGENTS.md` with `@.ai-style-rules.md` reference
...
- The instruction soft hook (`@.ai-style-rules.md`) is usually sufficient; hard hook only if the user wants mechanical enforcement

## Related Skills

- `codebase-onboarding` - analyze unfamiliar codebases and initialize guides
- `coding-standards` - cross-project baseline conventions and quality rules
