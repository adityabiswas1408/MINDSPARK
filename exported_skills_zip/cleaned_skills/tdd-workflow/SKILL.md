...
digraph tdd_cycle {
    ...
    red -> verify_red;
    verify_red -> green [label="fails as expected"];
    verify_red -> red [label="fails wrong reason\nor passes"];
    green -> verify_green;
    verify_green -> refactor [label="all green"];
    verify_green -> green [label="still failing"];
    refactor -> next [label="tests still pass"];
}
```

### Reference: Writing Good Tests
For in-depth test design rules (*Name the Break*, *Exercise the Real Thing*, *No Change Detectors*, *Table-driven Tests*), consult [`references/writing-good-tests.md`](file:///A:/billsplit/.agents/skills/tdd-workflow/references/writing-good-tests.md).
